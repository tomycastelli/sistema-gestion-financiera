...
export const messagesRouter = createTRPCRouter({
  createChat: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const response = await ctx.db.transaction(async (transaction) => {
        const [chatCreated] = await transaction
          .insert(chat)
        ...
        input.usersIds.forEach((str) =>
          relationsToInsert.push({ chatId: chatCreated.id, userId: str }),
        );
  ...
  getUserChats: protectedProcedure.query(async ({ ctx }) => {
    const response = await ctx.db.transaction(async (transaction) => {
      const userChats = await transaction
        .select({ id: chatToUsers.chatId, unseenMessages: count(messages.id) })
        .from(chatToUsers)
        .leftJoin(
          messages,
          and(
            eq(chatToUsers.chatId, messages.chatId),
            gt(messages.timestamp, chatToUsers.lastConnection),
          ),
        )
    ...
    const mappedChats = response.otherUsersInChats
      .filter((obj) => obj.userId !== ctx.user.id)
      .reduce((acc: MappedChat[], curr) => {
        ...
        if (existingChat) {
          ...
          acc.push({
            ...
            name: curr.name,
            unseenMessages:
              response.userChats.find((c) => c.id === curr.id)
                ?.unseenMessages ?? 0,
  ...
  getChatHistory: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const preparedWithPage = ctx.db
        ...
        .from(messages)
        .where(eq(messages.chatId, sql.placeholder("chat_id")))
        .orderBy(desc(messages.timestamp))
        .limit(sql.placeholder("queryLimit"))
        .offset(sql.placeholder("queryPage"))
        .prepare("getMessages");

      const preparedWithoutPage = ctx.db
        .select({
          id: messages.id,
          userId: messages.userId,
          timestamp: messages.timestamp,
      ...
      const redisMessageSchema = z.array(
        z.object({
          userid: z.string(),
          message: z.string(),
          timestamp: z.number(),
      ...
      const redisMessagesStrings = await ctx.redis.zrange(
        `chat:${input.chatId}`,
        "+inf",
        "-inf",
        "BYSCORE",
      ...
      if (redisMessagesStrings.length > 0) {
        ...
        if (input.limit - redisMessagesStrings.length <= 0) {
          const fakeIdRedisMessages = result.data.map((redisMsg, index) => ({
            id: index + 40,
          ...
          const dbMessages = await preparedWithoutPage.execute({
            chat_id: input.chatId,
            queryLimit: input.limit - redisMessagesStrings.length,
          });
        ...
        const dbMessages = await preparedWithPage.execute({
          chat_id: input.chatId,
          queryLimit: input.limit,
          queryPage: (input.page - 1) * input.limit,
        });
  ...
  sendMessage: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const res = await fetch(
        "https://" + env.CHAT_URL + `/message?chatId=${input.chatId}`,
        {
          method: "POST",
          headers: {
  ...
  updateChatLastConnection: protectedProcedure
    .input(
      z.object({
        chatId: z.number().int(),
        userId: z.string(),
        lastConnection: z.number().int(),
      }),
    )
    .mutation(async ({ ctx, input }) => {
      const [updatedChat] = await ctx.db
        .update(chatToUsers)
        .set({ lastConnection: input.lastConnection })
        .where(
          and(
