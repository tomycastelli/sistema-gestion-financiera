...
>(function UploadedUserOperations(
  ...
  const { data, isRefetching, isSuccess } =
    api.operations.getOperations.useQuery(
      { page: 1, limit: 5, uploadedById: user.id },
      { refetchOnWindowFocus: false, initialData: operationsData },
    );

  return !isRefetching ? (
    isSuccess && data.operations.length > 0 && (
      <div ref={ref} className="grid grid-cols-3 gap-4 lg:grid-cols-1">
        {data.operations.map((op) => (
          <Card key={op.id}>
            ...
            <CardContent>
              <p className="text-md text-muted-foreground">{op.observations}</p>
              <p className="text-md">
