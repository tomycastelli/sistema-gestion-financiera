        ...
        operation.isVisualizeAllowed ? (
          <Card className="w-[500px]">
            <CardHeader>
              <CardTitle>
                <Link
                  prefetch={false}
                  className="flex text-xl transition-all hover:scale-105"
            ...
            <CardContent className="grid grid-cols-1 gap-y-2">
              {operation.transactions.slice(0, 2).map((tx) => (
                <div key={tx.id} className="grid w-full grid-cols-5">
            ...
            <CardFooter className="flex justify-start text-muted-foreground">
              {operation.transactions.length > 1
                ? operation.transactions.length.toString() +
                  " " +
                  "transacciones"
                : "1 transacción"}
            </CardFooter>
          ...
          <Card className="w-[250px]">
            <CardHeader>
              <CardTitle>:(</CardTitle>
              <CardDescription>
                No tenés permisos para ver la operación
              </CardDescription>
            </CardHeader>
          </Card>
        )
