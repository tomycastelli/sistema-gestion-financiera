...
import { HoverCardTrigger, HoverCard, HoverCardContent } from '~/app/components/ui/hover-card'
...
const Chat: FC<ChatComponentProps> = ({ initialMessagesHistory, chatId, users, currentUser, chatUrl }) => {
  ...
  const { isRefetching, refetch } = api.messages.getChatHistory.useQuery({ chatId: chatId, limit: 15, page: historyPage }, {
    refetchOnWindowFocus: false,
    refetchOnReconnect: false,
    initialData: initialMessagesHistory,
    onSuccess(data) {
      setMessages([...data, ...messages].sort((a, b) => a.timestamp - b.timestamp))
      utils.messages.getUserChats.setData(undefined, old => old?.map(obj => {
        if (obj.id === chatId) {
  ...
  const { mutateAsync: updateLastConnection } = api.messages.updateChatLastConnection.useMutation()

  const { handleSubmit, control, reset } = form;

  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    await mutateAsync({ chatId: chatId, timestamp: Date.now(), message: data.messageText })
    reset()
  ...
  const { readyState } = useWebSocket(websocketURL, {
    ...
    onMessage: (event) => {
      ...
      if (!result.success) {
        ...
        setMessages((prev) => prev.concat({ id: 0, userId: result.data.userid, message: result.data.message, timestamp: result.data.timestamp }))
      }
    }
  })
  ...
  function parseOperationString(str: string): { valid: boolean } & (
  ...
  return (
    <div className="flex h-full flex-col gap-4 w-4/5 mx-auto rounded-xl">
      <ScrollArea className="rounded-xl border border-muted h-[550px] p-8">
        <div className="grid grid-cols-1 gap-1" ref={parent}>
          {isRefetching && <h2 className='text-lg font-semibold'>Cargando mensajes...</h2>}
          {canFetchMore && (
            <div className='w-full flex items-center justify-center'><Button onClick={() => setHistoryPage(historyPage + 1)}>Cargar mas</Button></div>
          )}
          {messages.map((msg, idx) => {
            const user = users.find(obj => obj.id === msg.userId)
            const isOperationInMessage = parseOperationString(msg.message)
            return (
              <div key={idx} className={cn("flex max-w-sm flex-col p-2 rounded-xl gap-1", msg.userId === currentUser.id ? "justify-self-end justify-end items-end border-r-muted border-r-4" : "justify-self-start justify-start items-start border-l-muted border-l-4")}>
                <div className={cn("flex gap-1 items-center", msg.userId === currentUser.id ? "justify-end flex-row" : "justify-start flex-row-reverse")}>
                  {!isOperationInMessage.valid ? (
                    <p className="text-lg">{msg.message}</p>
                  ) : (
                    <InlineOperation operationId={isOperationInMessage.number} />
                  )}
                  {user ? (
                ...
                <p className="text-md text-muted-foreground">{moment(msg.timestamp).format("DD-MM-YY HH:mm")}</p>
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                          className="bg-red"
                          onClick={() => deleteMessage({ chatId: chatId, timestamp: msg.timestamp, message: msg.message })}
                        >
                          Eliminar
                        </AlertDialogAction>
                      </AlertDialogFooter>
      ...
      <Form {...form}>
        <form onSubmit={handleSubmit(onSubmit)} className="w-full flex flex-row h-16 gap-2 items-center">
          <FormField
            control={control}
            name="messageText"
            render={({ field }) => (
          ...
          />
          <Button type='submit'><Icons.send className='w-5 h-5' /></Button>
          <HoverCard>
            <HoverCardTrigger asChild>
              <button>
                <Icons.globe className={cn('h-8 w-8', connectionStatus.color)} />
              </button>
            </HoverCardTrigger>
            <HoverCardContent className='flex justify-center items-center w-24 flex-wrap'>
