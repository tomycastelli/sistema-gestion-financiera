...
const BalancesCards: FC<BalancesCardsProps> = ({ balances, isInverted, accountType, selectedEntityId, selectedTag }) => {
  ...
  const transformedBalances: z.infer<typeof transformedBalancesSchema>[] = balances.reduce(
    (acc, balance) => {
      if (selectedEntityId) {
        ...
        const myOtherEntity = selectedTag === balance.selectedEntity.tagName ? balance.otherEntity : balance.selectedEntity
  ...
  const groupedTagBalances = transformedBalances.reduce((acc, balance) => {
    const myPOVTag = balance.entity.tagName

    let existingEntry = acc.find(obj => obj.tagName === balance.entity.tagName)
    ...
    balance.data.forEach(b => {
      let dataEntry = existingEntry!.data.find(d => d.currency === b.currency)
  ...
  return (
    <div className="flex flex-col gap-y-4">
      ...
      <div className="grid-cols grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
        {isGrouped ? groupedTagBalances.map(item => (
          <Card key={item.tagName} className="min-w-[300px] transition-all hover:scale-105 hover:shadow-md hover:shadow-primary">
            <Link
              prefetch={false}
              href={{
                pathname: "/cuentas",
